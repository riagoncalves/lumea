module Patients
  class AppointmentRoomsController < Patients::BaseController
    def show
      if appointment_details_service.call
        ...
        if @appointment.can_join_room?
          if video_calls_find_or_create_service.call
            ...
            redirect_to patient_appointment_path(@appointment.id), alert: video_calls_find_or_create_service.errors.full_messages.join('<br>')
          end
        else
          redirect_to patient_appointment_path(@appointment.id), alert: 'Video call cannot be started at this time.'
        end
      else
        redirect_to patient_appointments_path, alert: appointment_details_service.errors.full_messages.join('<br>')
      end
    end
